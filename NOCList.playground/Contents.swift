...
func sumCompromisedAgents(agents: [(coverName: String, realName: String, accessLevel: Int, compromised: Bool)]) -> Int {
    var totalCompromisedAgents = 0
    for agent in agents {
        // note - agent.3 would have worked to access last element of tuple
        if agent.compromised == true {
            totalCompromisedAgents += 1
//            print("\(agent.coverName) with Access Level \(agent.accessLevel) has been compromised.")
        } else {
//            print("\(agent.realName) hasn't been compromised.")
            //
        }
    }
...
let totalCompromisedAgents = sumCompromisedAgents(agents: agentArray)
...
let cleanAgents = findCleanAgents(agents: agentArray)

print("\(cleanAgents.count) clean agents out of \(agentArray.count) agents.")
...
func findHighRisk(agents: [(coverName: String, realName: String, accessLevel: Int, compromised: Bool)]) {
    for agent in agents {
        if agent.accessLevel >= 8 {
            var printString = agent.realName
            
            if agent.compromised == true {
                printString = printString + " **WARNING** **COMPROMISED**"
            }
            print(printString)
        }
    }
...
findHighRisk(agents: agentArray)
...
func agentLevelTotals(agents: [(coverName: String, realName: String, accessLevel: Int, compromised: Bool)]) {
...
agentLevelTotals(agents: agentArray)
...
func printSortedAgents(agents: [(coverName: String, realName: String, accessLevel: Int, compromised: Bool)]) {
    var sortedAgents = agents
    sortedAgents.sort(by: {$0.accessLevel < $1.accessLevel})
...
printSortedAgents(agents: agentArray)
